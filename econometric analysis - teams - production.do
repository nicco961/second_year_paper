...
esttab production_hospital_doctors production_clinic_doctors production_hospital_nurses production_clinic_nurses using production.tex, replace cells(b(star fmt(3)) se(par fmt(2))) keep(absences_team) ///
   legend label varlabels(_cons Constant)


***summary stats

eststo doctors_prod: quietly estpost summarize tot_clinic tot_hospital if job_description_code==15

eststo nurses_prod: quietly estpost summarize tot_clinic tot_hospital if job_description_code==5 | job_description_code==19

esttab doctors_prod nurses_prod using production_summary.tex , replace cells("mean(pattern(1 1 0) fmt(2)) sd(pattern(1 1 0)) b(star pattern(0 0 1) fmt(2)) p(pattern(0 0 1) par fmt(2))") 
